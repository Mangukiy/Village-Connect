...
Overview
This is a simple VillageConnect demo application built with Node.js, Express, and MySQL. It is designed for quick deployment to AWS (EC2 + RDS). The app allows adding/listing/deleting community services or announcements (title, description, contact).
...
Install Node.js 18+ and MySQL.
Create a database using create_table.sql.
...
Create an RDS MySQL instance. Note endpoint, username, password.
Create an EC2 instance (Ubuntu 22.04) and open ports 22 and 80 in its security group.
SSH to EC2 and run:
sudo apt update
...
npm start
Import DB schema from EC2 (or locally):
mysql -h <RDS_HOST> -u <user> -p < create_table.sql
Visit http://EC2_PUBLIC_IP:3000 (or set up nginx reverse proxy on port 80).
